class Game {
	constructor(pieces) {
		...
		this.pieces = pieces;
		this.playerOne = 'white';
		this.playerTwo = 'black';
		//this.turn    = 'white';
		//this.turnSign = document.getElementById('turn');
		this.blackReset = document.getElementById('blackReset');
		this.blackCD = document.getElementById('blackCooldown');
		this.whiteReset = document.getElementById('whiteReset');
		this.whiteCD = document.getElementById('whiteCooldown');
	...
	pieceMove(event) {
		...
		var name = null;
		if (this.getCooldownByColor(this.playerOne) == 0) {
	...
	getCooldownByColor(color) {
		if (color == 'black')
			return this.blackCD.innerHTML;
		else return this.whiteCD.innerHTML;
	}

	resetCooldown(color) {
		if (color == 'black') {
			this.blackReset.innerHTML = "true";
		}
		else {
			this.whiteReset.innerHTML = "true";
		}
	...
	getPieceAllowedMoves(event, pieceName){
		...
		if(/*this.turn*/this.playerOne == piece.color){
			this.clearSquares();
	...
	movePiece(event, square='') {
		...
		if (square.classList.contains('allowed')) {
			...
			const cooldown = this.getCooldownByColor(clickedPiece.color);
			if (clickedPiece && cooldown <= 0) {
				...
				this.clearSquares();
				//this.changeTurn(); //----------------------
				this.resetCooldown(clickedPiece.color);
				if (this.king_checked(/*this.turn*/this.playerTwo)) {
					if (this.king_dead(/*this.turn*/this.playerTwo)) {
